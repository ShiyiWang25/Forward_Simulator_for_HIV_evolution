...
def main():
	...
	filename, ori_seed, run_number, simulation_time, outfile_basename, rmt, mut_rates, mut_max_lengs, mut_block, assembly_name, species_name, sample_name, it_rate, ignore_warnings, titv = utilise_sysargs()  # checks the commandline arguments
	"""The main function."""
	if not filename:
		return False
...
def mutator(fasta, fai, mut_rates, mut_max_lengs, mut_block, rmt, outfile_basename, ref_filename, assembly_name, species_name, sample_name, titv):
	...
	chromosomes = list(fasta.keys())
	for i in trange(len(chromosomes), desc="Mutating Sequences", disable = True):
		no_tl_regions = []
		if rmt:
			mut_list = blist()
			#mut_list = []
...
def utilise_sysargs():
	...
	parser.add_argument("-ori_seed", help="Set original random seed")
	parser.add_argument("-run_number", help="define the number of simulations to run.")
	parser.add_argument("-simulation_time", help="Run time to pick the right random state")
	...
	filename = args.file
	ori_seed = int(args.ori_seed)
	if args.run_number:
		run_number = int(args.run_number)
		simulation_time = int(args.simulation_time)
	else:
		run_number = 1 # if not defined, run the simulation once
		simulation_time = 0
	...
	return filename, ori_seed, run_number, simulation_time, outfile_basename, rmt_file, mut_rates, mut_max_lengs, mut_block, assembly_name, species_name, sample_name, it_rate, ignore_warnings, titv
...
def save_mutations_vcf(vcf_name, ref_filename, fasta, chromosome, mut_list, assembly, species, sample, mode):
	...
	with open(vcf_name, mode) as hndl:
		...
		too_long = []
		for i in trange(len(mut_list), desc="Writing VCF", disable = True):
			entry = mut_list[i]
			#start, ref, alt, info = "", "", "", ""
			if entry[0] == "sn":
...
def get_mutations(start, stop, mut_rates, mut_max_lengs, mut_block):
	...
	#blocked_positions = []
	translocations = []
	pbar = trange(len(mut_positions), desc=f"Finding mutations in range: {start+1}-{stop+1}", disable = True)
	i=0
	mut_types = [rng.choice(mut_type_chances[0], p=mut_type_chances[1], size=len(mut_positions))]
...
def get_trans_inserts(translocations, data_length, blocked_positions, no_tl_regions):
	...
	if len(translocations) <= (data_length - len(blocked_positions)):
		positions = rng.choice(setdiff1d(arange(data_length), array(blocked_positions), True), len(translocations), replace=False)
		for i in trange(len(translocations), desc="Finding transloc inserts", disable = True):
		...
		positions = list(rng.choice(list(set(range(0, data_length)) - set(blocked_positions)), size = len(set(range(0, data_length)) - set(blocked_positions)), replace=False))
		rng.shuffle(translocations)
		for i in trange(len(translocations), desc="Finding transloc inserts", disable = True):
...
def get_mut_positions(start, stop, mut_rate):
	...
	positions = sorted(rng.choice(arange(start, stop + 1), mut_count, replace=False))
	return positions
...
def random_mutation_type(start, data_length, mut_max_lengs, mut_type):
	...
	if mutation[0] == "iv":  # needs a minimum mut_max_length of 2
		...
		stop = rng.integers(start + 1, start + (mut_max_lengs["iv"] - 1), endpoint=True)
		mutation.append(stop)
	elif mutation[0] == "du":
		#stop = rnd.randint(start, start + mut_max_lengs[mutation[0]] - 1)
		stop = rng.integers(start, start + mut_max_lengs[mutation[0]] - 1, endpoint=True)
		if stop > data_length:
			stop = data_length
...
def transloc_invert(transloc_len):
	...
	if rng.integers(0, 1, endpoint=True) == 0 or transloc_len <= 1:
		return False
	else:
		return True
...
def lib_partial(lib, di_number):
	lib_partial = {}
	
	lib_keys = list(lib.keys())
	keys_partial = rng.choice(lib_keys, di_number, replace=False)
	
	for key in keys_partial:
		lib_partial[key] = lib[key]
...
def interchromosomal_transloc(fasta, rate, rmt):
	...
	for chrom in avail_chr: # find partner chromosome
		...
		if len(avail_chr)>0:
			partner = avail_chr[rng.integers(0, len(avail_chr))]
			partners[chrom]=partner
			avail_chr.remove(partner)
	partners = lib_partial(partners, int(len(records)/6)) #20%
	#print(f'dual-infection#: {len(partners)}')
	...
	for chrom in chr_to_cross: #apply interchromosomal translocations
		...
		if rmt:
			...
			amount = int((len(records[chrom])+len(records[partners[chrom]])-4)/2 * rate) # 220319, 2e-3 per site per generation
		try:
			pos_chrom = []
			...
			#pos_partner = sorted(sample_with_minimum_distance(len(records[partners[chrom]]), amount, 2)) # my adds-on
			#pos_partner = pos_chrom # my adds-on

			#print("RR interval used.")
			for key, value in interval_rr.items(): # 220319
				for i in range(int(key.split('-')[0]), int(key.split('-')[1]) + 1):
					if rng.random() < value/1000:
						pos_chrom.append(i)
					else: pass
			pos_partner = pos_chrom # my adds-on
			#print(f'{len(pos_partner)} breakpoints in {chrom}:{partners[chrom]}')
		except ValueError:
...
def subdicts_empty(dict):
	...
	for subdict in dict:
		if dict[subdict] != []: # my adds-on
			if dict[subdict]["self"]:
				is_empty=False
				pass
	return is_empty
...
def mutate(fasta, mut_list, titv):
	...
	data = blist(fasta[:])
	#data = list(fasta[:])
	for i in trange(len(mut_list), desc="Mutating", disable = True):
		if mut_list[i][0] == "sn":
			...
			#data=data[:mut_list[i][1]]+list(insert)+data[mut_list[i][1]:]
			...
			#invert = list(data[mut_list[i][1]:mut_list[i][2] + 1])
			del (data[mut_list[i][1]:mut_list[i][2] + 1])
			...
			#dupe = list(data[mut_list[i][1]:mut_list[i][2] + 1])
			...
			#insert = list(fasta[mut_list[i][2]:mut_list[i][3] + 1])
			if mut_list[i][4]:
...
def get_tv_Base(base):
	...
	return {"A": ["T", "C"], "G": ["C", "T"], "T": ["G", "A"], "C": ["A", "G"], "N": ["N", "N"]}[base][rng.integers(2)] # replace randint
...
def get_insert(leng):
	...
	return "".join(rng.choice(["A", "T", "G", "C"], leng))
...
def write_fasta(filename, chromosome, header, linebases, mode):
	...
	with open(filename, mode) as hndl:
		hndl.write(f">{header}\n")
		for i in trange(0,len(chromosome),linebases,desc="Writing Sequence", disable = True):
...
def sample_with_minimum_distance(n=40, k=4, d=10):
	...
	sample = list(rng.integers(1, n-(k-1)*(d-1), k))
...
if __name__ == "__main__":
    try:
        ...
        child_states = ss.spawn(run_number)
        
        random_state = child_states[simulation_time]
        rng = stochastic_function(random_state) # RNG set.
        main()
